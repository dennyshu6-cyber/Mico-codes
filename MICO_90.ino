...
// Previous step区：d < 4cm
const float PREV_CM = 4.0;

// Next-step zone: 9–15cm
const float NEXT_MIN_CM = 9.0;
const float NEXT_MAX_CM = 15.0;

// Anti-repeat trigger (to prevent a single action from skipping multiple steps)
const unsigned long NAV_COOLDOWN_MS = 1200;

// Used for “edge triggering”: it triggers only once when entering a specific zone
...
if (Serial.available() > 0) {
  ...
  switch (currentState) {
    ...
    case STEP4:
    {
      // ----- First time entering STEP4: show the original 'STEP4 + image' -----
      if (millis() - lastStateChangeTime < 50) { // First entry
        step4Phase = STEP4_SHOW_IMAGE;          // When entering STEP4, first return to the image view
      ...
      // ----- If in 'timing' state, update the progress bar every frame -----
      if (step4Phase == STEP4_TIMING) {
        ...
        if (elapsed >= STEP4_DURATION_MS) {
          ...
          Serial.println("TIMER_UP"); // Timer finished, wait for next button press to enter STEP5
          if (!step4DoneSoundPlayed) {
          step4DoneSoundPlayed = true;
          player.stop();     // Stop the currently looping 0004
          player.play(6);    // Play completion sound 0005 (once)
  }
        }
      ...
      // ----- Button logic: three phases -----
      if (buttonPressed) {
        // First press in STEP4: switch from image to progress bar and start timing
        if (step4Phase == STEP4_SHOW_IMAGE) {
          step4Phase = STEP4_TIMING;
          step4StartMs = millis();
          showStep4ProgressUI(0.0f); // Immediately show progress bar
          player.loop(4);  
        }

        // After timing ends, press again to enter STEP5
        ...
        // If timing is in progress, ignore button press (do not jump steps)
      }
  ...
  //  Change to 'press button once to enter check state', and consume this button event to avoid jumping to COUNTDOWN after returning HOME
  if (buttonPressed) {
    buttonPressed = false;
    player.stop();              // Stop alarm sound (if currently playing)
  ...
  // If air improves at any moment within 10 seconds, count as success
  if (!airIsBad) {
    airImprovedDuringRecheck = true;
  }

  // After 10 seconds: success returns to previous state; failure returns to alarm
...
if (currentState == CONGRATS) {
  if (millis() - congratsLastLedMs > 200) {  // Change color every 200ms
...
void changeState(SystemState newState) {
  ...
  if (newState >= STEP1 && newState <= STEP8) {
    player.play(1);   // Play 0001.mp3 (same as the boot sound using player.play(3))
  } */
  ...
  switch(newState) {
    ...
    case STEP4:
        // Every time entering STEP4, return to the initial 'show image' phase
...
// Read ultrasonic distance once (cm), return -1 on failure
float readUltrasonicCM() {
  ...
  // 30ms timeout (~5m) to prevent blocking
...
void handleGesture() {
  // Respond only on STEP pages (keep original design)
  ...
  bool entering = (zone != lastZone);                    // Entering a new zone
  bool cooledDown = (now - lastNavMs > NAV_COOLDOWN_MS); // Cooldown finished

  if (entering && cooledDown) {

    // --- Previous-step zone: d < 5cm ---
    if (zone == Z_PREV) {
      gestureRight = true; // Previous step
      lastNavMs = now;
    }

    // --- Next-step zone: 9–15cm ---
    if (zone == Z_NEXT) {
      // If you still want STEP4 to advance only via button, keep this line; otherwise remove the if
      if (currentState != STEP4) {
        gestureLeft = true; // Next step
        lastNavMs = now;
      }
    }
  }
...
void renderProgressBar(float progress) {
  ...
  // Title: Checking Air (at the top)
  display.setTextSize(1);
  display.setTextColor(SH110X_WHITE);
  display.setCursor(0, 0);
  display.print("Checking Air");

  // Progress bar placed below (to avoid overlapping the title)
...
void flashStrip() {
  ...
  player.disableLoop();   // Key: disable DFPlayer loop mode (to prevent play(1) from looping as well)
  //player.stop();          // Optional but recommended: interrupt the previous looping sound
  player.play(1);   
}